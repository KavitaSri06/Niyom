...
function PriceTable({ prices, commodity }: { prices: CommodityPrice[]; commodity: 'gold' | 'silver' }) {
  if (prices.length === 0) {
    return <p className="text-gray-500 text-sm italic">No price data available for this month.</p>;
  }
    ...
    <div>
      ...
      <div className={`mb-4 p-4 rounded-lg border-l-4 ${commodity === 'gold' ? 'bg-gradient-to-r from-yellow-50 to-yellow-100 border-yellow-500' : 'bg-gradient-to-r from-gray-100 to-gray-200 border-gray-500'}`}>
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            <p className={`font-bold text-xl ${commodity === 'gold' ? 'text-yellow-900' : 'text-gray-900'}`}>
              Latest MCX Price: {formatPrice(latest.price, commodity)}
            </p>
            <p className={`text-sm mt-0.5 ${commodity === 'gold' ? 'text-yellow-700' : 'text-gray-600'}`}>
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200">
        <table className="w-full text-sm">
          <thead>
            <tr className={`${commodity === 'gold' ? 'bg-yellow-50' : 'bg-gray-100'}`}>
              <th className="text-left px-4 py-2 font-semibold text-gray-700">Date</th>
              <th className="text-right px-4 py-2 font-semibold text-gray-700">MCX Price</th>
              <th className="text-right px-4 py-2 font-semibold text-gray-700">Change</th>
            </tr>
          </thead>
          <tbody>
            {prices.map((row, i) => {
              ...
              return (
                <tr key={row.price_date} className={`border-t border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-opacity-80 transition-colors`}>
                  <td className="px-4 py-2 text-gray-700 font-medium">{formatDate(row.price_date)}</td>
                  <td className="px-4 py-2 text-right font-semibold text-gray-900">{formatPrice(row.price, commodity)}</td>
                  <td className={`px-4 py-2 text-right text-xs font-medium ${dayChange > 0 ? 'text-green-600' : dayChange < 0 ? 'text-red-600' : 'text-gray-400'}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h4 className="text-xl font-bold text-black">Daily MCX Prices — {getMonthName()}</h4>
          {lastUpdated && <p className="text-xs text-gray-500 mt-0.5">Last fetched: {lastUpdated}</p>}
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={refetch}
            disabled={loading}
            className="flex items-center gap-1.5 text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50"
          >
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <RefreshCw size={24} className="animate-spin text-[#c9b896] mr-3" />
          <span className="text-gray-600">Fetching MCX prices…</span>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h5 className="font-bold text-lg text-[#c9b896]">Gold (per 10g)</h5>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h5 className="font-bold text-lg text-[#c9b896]">Silver (per kg)</h5>
            </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h5 className="font-bold text-lg text-[#c9b896] mb-3">Gold — Historical Ranges</h5>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h5 className="font-bold text-lg text-[#c9b896] mb-3">Silver — Historical Ranges</h5>
          <ul className="space-y-1.5 text-gray-700 text-sm">
      ...
      {/* Key factors */}
      <div>
        <h4 className="text-xl font-bold text-black mb-3">Key Factors Driving Price Changes</h4>
        <div className="grid md:grid-cols-2 gap-4">
          <div className="bg-white p-5 rounded-lg shadow-md">
            <h5 className="font-bold text-[#c9b896] mb-2">Inflation & Currency Weakness</h5>
            <p className="text-gray-700">Rising inflation and weakening rupee increased demand for gold and silver as safe-haven assets.</p>
          </div>
          <div className="bg-white p-5 rounded-lg shadow-md">
            <h5 className="font-bold text-[#c9b896] mb-2">Global Economic Uncertainty</h5>
            <p className="text-gray-700">COVID-19 pandemic, geopolitical tensions, and economic slowdowns drove investors to precious metals.</p>
          </div>
          <div className="bg-white p-5 rounded-lg shadow-md">
            <h5 className="font-bold text-[#c9b896] mb-2">Central Bank Policies</h5>
            <p className="text-gray-700">Low interest rates and quantitative easing increased precious metal attractiveness over bonds.</p>
          </div>
          <div className="bg-white p-5 rounded-lg shadow-md">
            <h5 className="font-bold text-[#c9b896] mb-2">Industrial Demand</h5>
            <p className="text-gray-700">Growing use in electronics, solar panels, and technology sectors boosted silver demand.</p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-5xl font-bold text-black mb-4" style={{ fontFamily: 'Playfair Display, Georgia, serif' }}>
              Financial <span className="text-[#c9b896]">Education Center</span>
            </h2>
            <div className="w-32 h-1 bg-[#c9b896] mx-auto mb-6"></div>
          ...
          <div className="space-y-6">
            ...
            >
              <div className="space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-[#c9b896]">
                    <h5 className="font-bold text-lg text-black mb-2">Equity Funds</h5>
                    <p className="text-gray-700">Invest primarily in stocks. Higher risk but potential for higher returns. Best for long-term goals (5+ years).</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-[#c9b896]">
                    <h5 className="font-bold text-lg text-black mb-2">Debt Funds</h5>
                    <p className="text-gray-700">Invest in bonds and fixed-income securities. Lower risk, stable returns. Suitable for short to medium-term goals.</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-[#c9b896]">
                    <h5 className="font-bold text-lg text-black mb-2">Hybrid Funds</h5>
                    <p className="text-gray-700">Mix of equity and debt. Balanced risk-return profile. Good for moderate risk-takers seeking diversification.</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-[#c9b896]">
                    <h5 className="font-bold text-lg text-black mb-2">Index Funds</h5>
                    <p className="text-gray-700">Track a market index like Nifty 50 or Sensex. Low cost, passive investment matching market performance.</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-[#c9b896]">
                    <h5 className="font-bold text-lg text-black mb-2">ELSS (Tax Saving)</h5>
                    <p className="text-gray-700">Equity-linked savings schemes with tax benefits under Section 80C. 3-year lock-in period.</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-[#c9b896]">
                    <h5 className="font-bold text-lg text-black mb-2">Liquid Funds</h5>
                    <p className="text-gray-700">Invest in very short-term instruments. Highest liquidity with minimal risk. Alternative to savings accounts.</p>
                  </div>
                </div>
              </div>
            </LearningSection>
            ...
            >
              <div className="space-y-4">
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                  <h5 className="font-bold text-lg text-black mb-3">Key Features</h5>
              ...
              <div className="space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-blue-500">
                    <h5 className="font-bold text-lg text-black mb-2">Term Life Insurance</h5>
                    <p className="text-gray-700 mb-3">Pure protection plan with high coverage at low premiums. No maturity benefit. Best for income protection.</p>
                  ...
                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-purple-500">
                    <h5 className="font-bold text-lg text-black mb-2">Endowment Plans</h5>
                    <p className="text-gray-700 mb-3">Combination of insurance and savings. Returns premium with bonus at maturity. Lower coverage, higher premiums.</p>
                    <p className="text-sm text-purple-600 font-semibold">Returns: 5-6% typically, with guaranteed benefits</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-green-500">
                    <h5 className="font-bold text-lg text-black mb-2">ULIPs</h5>
                    <p className="text-gray-700 mb-3">Unit Linked Insurance Plans combine insurance with market-linked investments. Flexible and transparent.</p>
                    <p className="text-sm text-green-600 font-semibold">Lock-in: 5 years, potential for market-linked returns</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-red-500">
                    <h5 className="font-bold text-lg text-black mb-2">Health Insurance</h5>
                    <p className="text-gray-700 mb-3">Covers medical expenses, hospitalization, and treatments. Essential for managing healthcare costs.</p>
                    <p className="text-sm text-red-600 font-semibold">Coverage: ₹5-25 lakh typically recommended per family</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-yellow-600">
                    <h5 className="font-bold text-lg text-black mb-2">Critical Illness Insurance</h5>
                    <p className="text-gray-700 mb-3">Lump sum payout on diagnosis of specified critical illnesses like cancer, heart attack, stroke.</p>
                    <p className="text-sm text-yellow-700 font-semibold">Covers 30-40 critical illnesses typically</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-indigo-500">
                    <h5 className="font-bold text-lg text-black mb-2">Motor & Property Insurance</h5>
                    <p className="text-gray-700 mb-3">Protects vehicles and property from damage, theft, or loss. Mandatory for vehicles in India.</p>
            ...
            >
              <div className="space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="bg-white p-6 rounded-lg shadow-md">
                    <h5 className="font-bold text-lg text-[#c9b896] mb-2">Government Securities (G-Secs)</h5>
                    <p className="text-gray-700 mb-2">Issued by Government of India. Virtually zero default risk. Returns: 6.5-7.5% typically.</p>
                    <p className="text-sm text-gray-600"><strong>Best for:</strong> Conservative investors seeking safety</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md">
                    <h5 className="font-bold text-lg text-[#c9b896] mb-2">Corporate Bonds (AAA-Rated)</h5>
                    <p className="text-gray-700 mb-2">Issued by top-rated companies. Low risk with better returns than G-Secs. Returns: 7.5-9%.</p>
                    <p className="text-sm text-gray-600"><strong>Examples:</strong> HDFC, Reliance, TCS bonds</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md">
                    <h5 className="font-bold text-lg text-[#c9b896] mb-2">Tax-Free Bonds</h5>
                    <p className="text-gray-700 mb-2">Issued by government entities. Interest income exempt from tax. Returns: 5-6% (tax-free).</p>
                    <p className="text-sm text-gray-600"><strong>Issuers:</strong> NHAI, IRFC, PFC, REC</p>
                  </div>

                  <div className="bg-white p-6 rounded-lg shadow-md">
                    <h5 className="font-bold text-lg text-[#c9b896] mb-2">High-Yield Bonds</h5>
                    <p className="text-gray-700 mb-2">Lower-rated bonds (BB and below) offering higher returns. Higher risk. Returns: 10-14%.</p>
                    <p className="text-sm text-gray-600"><strong>Risk:</strong> Higher default probability, suitable for experienced investors</p>
                  </div>
                </div>
            ...
            >
              <div className="space-y-6">
                ...
                <div>
                  <h4 className="text-2xl font-bold text-black mb-4">Historical Success Stories (Last 5-10 Years)</h4>

                  <div className="space-y-4">
                    <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-green-500">
                      <h5 className="font-bold text-xl text-black mb-2">Zomato (Food Delivery)</h5>
                      <div className="grid md:grid-cols-2 gap-4 mt-3">
                        <div>
                          <p className="text-gray-700"><strong>Pre-IPO Price (2021):</strong> ₹60-70 per share</p>
                          <p className="text-gray-700"><strong>IPO Price (July 2021):</strong> ₹76 per share</p>
                          <p className="text-gray-700"><strong>Listing Price:</strong> ₹116 per share</p>
                          <p className="text-gray-700"><strong>Current Range (2024):</strong> ₹160-280 per share</p>
                        </div>
                        <div className="bg-green-50 p-4 rounded">
                          ...
                          <p className="text-sm text-gray-600 mt-2">Early investors saw exceptional returns as company established market dominance.</p>
                        </div>
                      </div>
                    </div>

                    <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-green-500">
                      <h5 className="font-bold text-xl text-black mb-2">Paytm (Fintech)</h5>
                      <div className="grid md:grid-cols-2 gap-4 mt-3">
                        <div>
                          <p className="text-gray-700"><strong>Pre-IPO Price (2021):</strong> ₹1,850-2,000 per share</p>
                          <p className="text-gray-700"><strong>IPO Price (Nov 2021):</strong> ₹2,150 per share</p>
                          <p className="text-gray-700"><strong>Listing Price:</strong> ₹1,950 per share</p>
                          <p className="text-gray-700"><strong>Current Range (2024):</strong> ₹400-900 per share</p>
                        </div>
                        <div className="bg-red-50 p-4 rounded">
                          ...
                          <p className="text-sm text-gray-600 mt-2">Cautionary tale of overvaluation and market sentiment.</p>
                        </div>
                      </div>
                    </div>

                    <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-green-500">
                      <h5 className="font-bold text-xl text-black mb-2">Nykaa (E-commerce Beauty)</h5>
                      <div className="grid md:grid-cols-2 gap-4 mt-3">
                        <div>
                          <p className="text-gray-700"><strong>Pre-IPO Price (2021):</strong> ₹800-900 per share</p>
                          <p className="text-gray-700"><strong>IPO Price (Oct 2021):</strong> ₹1,125 per share</p>
                          <p className="text-gray-700"><strong>Listing Price:</strong> ₹2,001 per share</p>
                          <p className="text-gray-700"><strong>Current Range (2024):</strong> ₹160-210 per share</p>
                        </div>
                        <div className="bg-yellow-50 p-4 rounded">
                          ...
                          <p className="text-sm text-gray-600 mt-2">Spectacular listing, but long-term performance challenged by market correction.</p>
                        </div>
                      </div>
                    </div>

                    <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-green-500">
                      <h5 className="font-bold text-xl text-black mb-2">PolicyBazaar/PB Fintech</h5>
                      <div className="grid md:grid-cols-2 gap-4 mt-3">
                        <div>
                          <p className="text-gray-700"><strong>Pre-IPO Price (2021):</strong> ₹750-850 per share</p>
                          <p className="text-gray-700"><strong>IPO Price (Nov 2021):</strong> ₹980 per share</p>
                          <p className="text-gray-700"><strong>Listing Price:</strong> ₹1,150 per share</p>
                          <p className="text-gray-700"><strong>Current Range (2024):</strong> ₹1,300-1,800 per share</p>
                        </div>
                        <div className="bg-green-50 p-4 rounded">
                          ...
                          <p className="text-sm text-gray-600 mt-2">Solid performer with strong fundamentals and market position.</p>
                        </div>
                      </div>
                    </div>

                    <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-blue-500">
                      <h5 className="font-bold text-xl text-black mb-2">Unlisted Giants (Still Private)</h5>
                      <div className="space-y-3 mt-3">
                        <div className="bg-blue-50 p-4 rounded">
                          <p className="font-bold text-blue-900">BYJU'S</p>
                          <p className="text-gray-700"><strong>Trading Range (2020-2022):</strong> ₹300-700 per share</p>
                          <p className="text-gray-700"><strong>Peak Valuation:</strong> ₹1,80,000 crore (2022)</p>
                          <p className="text-gray-700"><strong>Current Status:</strong> Facing significant challenges, valuation dropped ~90%</p>
                        </div>

                        <div className="bg-blue-50 p-4 rounded">
                          <p className="font-bold text-blue-900">OYO Rooms</p>
                          <p className="text-gray-700"><strong>Unlisted Price Range (2019-2023):</strong> ₹120-200 per share</p>
                          <p className="text-gray-700"><strong>Status:</strong> Filed for IPO multiple times, still unlisted</p>
                          <p className="text-gray-700">Volatile valuations based on funding rounds and business performance</p>
                        </div>

                        <div className="bg-blue-50 p-4 rounded">
                          <p className="font-bold text-blue-900">Flipkart (acquired by Walmart)</p>
                          <p className="text-gray-700"><strong>Pre-Acquisition Trading:</strong> ₹80-150 per share (2015-2018)</p>
                          <p className="text-gray-700"><strong>Acquisition (2018):</strong> ₹1,31,000 crore valuation</p>
                          <p className="text-gray-700">Early unlisted investors made 10-20x returns through acquisition</p>
                        </div>
                      </div>
                    </div>

                    <div className="bg-white p-6 rounded-lg shadow-md border-l-4 border-[#c9b896]">
                      <h5 className="font-bold text-xl text-black mb-2">Premium Unlisted Opportunities</h5>
                      <div className="space-y-3 mt-3">
                        <div className="bg-gradient-to-br from-amber-50 to-amber-100 p-4 rounded">
                          ...
                          <p className="text-gray-700"><strong>Type:</strong> Stock Exchange Infrastructure</p>
                          <p className="text-gray-700"><strong>Unlisted Price Range (Feb 2026):</strong> ₹1,980 - ₹2,120 per share</p>
                          <p className="text-gray-700"><strong>52-Week Range:</strong> ₹1,650 - ₹2,470 per share</p>
                          <p className="text-gray-700"><strong>Business:</strong> India's largest stock exchange by market cap, handling billions in daily transactions</p>
                          <p className="text-gray-700"><strong>Key Highlights:</strong> Monopolistic position, strong revenue from transaction fees, regulatory backing</p>
                        ...
                        <div className="bg-gradient-to-br from-amber-50 to-amber-100 p-4 rounded">
                          ...
                          <p className="text-gray-700"><strong>Type:</strong> Sports & Entertainment</p>
                          <p className="text-gray-700"><strong>Unlisted Price Range (Feb 2026):</strong> ₹240 - ₹271 per share</p>
                          <p className="text-gray-700"><strong>Face Value:</strong> ₹0.10 per share | <strong>Lot Size:</strong> 100 shares</p>
                          <p className="text-gray-700"><strong>Business:</strong> Premier IPL cricket franchise with massive fan following and brand value</p>
                          <p className="text-gray-700"><strong>Key Highlights:</strong> 5x IPL champions, strong revenue from broadcasting, sponsorships, merchandise</p>
                          <p className="text-gray-700"><strong>Financials:</strong> Annual revenue ₹500-600 crores, consistent profitability</p>
                        ...
                        <div className="bg-gradient-to-br from-amber-50 to-amber-100 p-4 rounded">
                          ...
                          <p className="text-gray-700"><strong>Type:</strong> Stock Exchange</p>
                          <p className="text-gray-700"><strong>Unlisted Price Range (Feb 2026):</strong> ₹5.20 - ₹5.44 per share</p>
                          <p className="text-gray-700"><strong>52-Week Range:</strong> ₹2.00 - ₹9.00 per share | <strong>Face Value:</strong> ₹1.00</p>
                          <p className="text-gray-700"><strong>Business:</strong> National-level stock exchange recognized by SEBI, trading platform for equity, derivatives, debt, and currency</p>
                          <p className="text-gray-700"><strong>Key Highlights:</strong> SEBI recognized, Liquidity Enhancement Scheme launched in Jan 2026, technology-driven operations</p>
