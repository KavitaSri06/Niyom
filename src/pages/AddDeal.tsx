...
const PRODUCT_TYPES = [
  ...
  { value: 'bonds', label: 'Bonds', color: 'bg-bg-base border-border text-text-secondary hover:bg-bg-raised', active: 'bg-slate-700 border-slate-700 text-white' },
  ...
  { value: 'other', label: 'Other', color: 'bg-bg-base border-border text-text-secondary hover:bg-bg-raised', active: 'bg-gray-700 border-gray-700 text-white' },
];

const STATUS_OPTIONS = [
  { value: 'pending', label: 'Pending', active: 'bg-amber-500 border-amber-500 text-white', inactive: 'border-border text-text-secondary hover:border-amber-300 hover:bg-amber-50' },
  { value: 'closed', label: 'Closed', active: 'bg-emerald-600 border-emerald-600 text-white', inactive: 'border-border text-text-secondary hover:border-emerald-300 hover:bg-emerald-50' },
  { value: 'cancelled', label: 'Cancelled', active: 'bg-red-600 border-red-600 text-white', inactive: 'border-border text-text-secondary hover:border-red-300 hover:bg-red-50' },
];
  ...
  if (success) {
    return (
      <div className="min-h-screen bg-bg-base flex items-center justify-center p-4">
        <div className="bg-bg-elevated rounded-2xl border border-border shadow-sm p-10 text-center max-w-sm w-full">
          <div className="w-16 h-16 bg-emerald-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <CheckCircle2 className="w-8 h-8 text-emerald-600" />
          </div>
          <h2 className="text-lg font-bold text-text-primary mb-1">Deal Added!</h2>
          <p className="text-sm text-text-muted">Redirecting to your dashboard...</p>
          <div className="mt-4 w-8 h-8 border-2 border-border border-t-gray-900 rounded-full animate-spin mx-auto" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-bg-base/50">
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6">
          <div className="flex items-center h-14 gap-3">
            <button onClick={() => window.location.href = '/crm/employee'}
              className="p-2 hover:bg-bg-raised rounded-xl transition-colors text-text-muted hover:text-text-primary">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <div className="h-4 w-px bg-bg-raised" />
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 bg-gray-900 rounded-lg flex items-center justify-center">
                <Briefcase className="w-3 h-3 text-white" />
              </div>
              <span className="text-sm font-bold text-text-primary">Record a Deal</span>
            </div>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-xl font-bold text-text-primary">New Deal</h1>
          <p className="text-sm text-text-muted mt-0.5">{employee?.full_name} · {employee?.level}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {/* Client Name */}
          <div className="bg-bg-elevated rounded-2xl border border-border p-5 shadow-sm">
            <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-3">Client Name</label>
            <input type="text" value={formData.client_name}
              onChange={e => setFormData({ ...formData, client_name: e.target.value })}
              className="w-full px-4 py-2.5 border border-border rounded-xl focus:ring-2 focus:ring-gray-900 focus:border-transparent text-sm bg-bg-base focus:bg-bg-elevated transition-colors"
          ...
          <div className="bg-bg-elevated rounded-2xl border border-border p-5 shadow-sm">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-text-muted mt-1.5">Commission / brokerage earned</p>
              </div>
            </div>
          </div>
          ...
          {/* Notes */}
          <div className="bg-bg-elevated rounded-2xl border border-border p-5 shadow-sm">
            <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-3">Notes</label>
            <textarea value={formData.notes} onChange={e => setFormData({ ...formData, notes: e.target.value })}
              rows={3}
              className="w-full px-4 py-2.5 border border-border rounded-xl focus:ring-2 focus:ring-gray-900 focus:border-transparent text-sm bg-bg-base focus:bg-bg-elevated transition-colors resize-none"
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => window.location.href = '/crm/employee'}
              className="flex-1 px-6 py-3 border border-border text-text-secondary rounded-xl hover:bg-bg-base transition-colors text-sm font-semibold">
              Cancel
            </button>
