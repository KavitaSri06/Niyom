/*
  # Lead Management — dashboard RPC fix 2 (variable/column collision)

  The PL/pgSQL variable `today` collided with the `totals.today` output column in
  the final SELECT, which can raise "column reference is ambiguous" depending on
  the server's plpgsql.variable_conflict setting. Renamed the variable to
  `today_d` so no bare identifier is ambiguous. Logic is otherwise identical to
  20260719180000.
*/
...
-- Self-test: exercise the full aggregation (admin scope) so any runtime SQL
-- error fails this migration instead of surfacing as a broken dashboard. The
-- result is discarded; nothing is written or returned.
DO $$
DECLARE
  adm boolean := true; me uuid := NULL; today_d date := (now() AT TIME ZONE 'Asia/Kolkata')::date; r jsonb;
BEGIN
  WITH scoped AS (SELECT * FROM nw_leads WHERE adm OR owner_employee_id = me OR created_by_employee_id = me)
  SELECT jsonb_build_object(
    'monthly_trend', (SELECT COALESCE(jsonb_agg(jsonb_build_object('label', to_char(m,'Mon'), 'count', COALESCE(c,0)) ORDER BY m), '[]')
                        FROM generate_series(date_trunc('month', now()) - interval '5 months', date_trunc('month', now()), interval '1 month') m
                        LEFT JOIN (SELECT date_trunc('month', created_at) mm, count(*) c FROM scoped GROUP BY 1) t ON t.mm = m),
    'daily_calls', (SELECT COALESCE(jsonb_agg(jsonb_build_object('label', to_char(dd,'Dy'), 'count', COALESCE(c,0)) ORDER BY dd), '[]')
                      FROM (SELECT (today_d - g)::date dd FROM generate_series(0,6) g) days
                      LEFT JOIN (SELECT c.created_at::date cdate, count(*) c FROM nw_lead_communications c WHERE c.comm_type='call' GROUP BY 1) t ON t.cdate = days.dd),
    'by_employee', (SELECT COALESCE(jsonb_agg(jsonb_build_object('label', name, 'total', tot) ORDER BY tot DESC), '[]')
          FROM (SELECT e.full_name name, count(l.*) tot FROM nw_employees e JOIN nw_leads l ON l.owner_employee_id = e.id GROUP BY e.full_name LIMIT 10) s),
    'funnel', (SELECT COALESCE(jsonb_agg(jsonb_build_object('label', lbl, 'count', c) ORDER BY ord), '[]')
                 FROM (SELECT 1 ord, 'x' lbl, count(*) FILTER (WHERE status='New') c FROM scoped) s)
  ) INTO r;
  RAISE NOTICE 'nw_lead_dashboard self-test OK';
END $$;
